...
-   Step 1: Click on the `+` button to expand the course items' view.
-   Step 2: Click on `SCORM Package`. This will add a SCORM item inside your section.
-   Step 3: Click on `Attach Package`, upload your `.zip`, and wait for it to process.
-   Step 4: Configure display options (open in a new window or embed inline).
-   Step 5: Click on `Save`, then publish the item to make it available to students.
