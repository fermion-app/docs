...
You must have a valid Fermion API key to run these coding labs. Create a Fermion account and go to Settings > API access section in your instructor dashboard to get the key. Learn how to get your [API key here](/api-guide/introduction#how-to-get-your-api-key)

## Base64URL Encoding

Base64URL encoding is **critical** for proper API usage. The DSA execution API requires Base64URL encoding (not standard Base64) for several fields including **source code**, **expected output**, **stdin input**, and **file attachments** for [SQL labs](/creating-io-coding-labs/sql-over-api).
