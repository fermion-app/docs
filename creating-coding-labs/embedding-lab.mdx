...
Complete documentation has been laid out [here](/creating-coding-labs/what-are-coding-labs), where you can follow the instructions according to the technology you want to use, and create coding labs.
...
const jwtToken = jwt.sign(
	{
		...
		playgroundOptions: {
			...
			// change this to true to hide the test output logs section in the editor.
			shouldHideTestOutputLogs: false,
...
### shouldHideTestOutputLogs

By default, the output logs of the lab's challenges will be shown in the embedded IDE after users run their code. If you set `shouldHideTestOutputLogs` to `true`, the test output logs section will be hidden from the user's view.

**Example:**
```js
playgroundOptions: {
    shouldHideTestOutputLogs: true,
}
```
### isReadOnly

Setting `isReadOnly` to `true` will render the embedded coding lab in a read-only mode. This means users can view the lab and its content, but will not be able to edit files. This is useful if you want to showcase the lab without allowing further interaction.
...
> **Note:** After creating a new lab or updating its filesystem programmatically (for example, via API or automation), please allow up to **2 minutes** before starting the lab. This delay ensures that all lab files and environment are fully synchronized and ready for use. Starting the lab immediately after creation may result in incorrect or outdated file contents(when override is enabled).
