...
When you call `disconnect()`, Fermion performs a clean shutdown: it disables automatic reconnection, sends a disconnection signal to the container, closes all open communication channels, and resets the client’s internal state. Depending on the sandbox type, the container will either be **destroyed** (for ephemeral sessions) or **paused and saved** (for persistent sessions created with `shouldBackupFilesystem: true`).  
